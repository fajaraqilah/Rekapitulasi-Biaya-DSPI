...
CREATE TABLE IF NOT EXISTS beban_biaya_master (
  ...
  UNIQUE (tahun, kategori, subkategori)
);
...
-- Create bpd_budget_master table
CREATE TABLE IF NOT EXISTS bpd_budget_master (
  id uuid PRIMARY KEY DEFAULT uuid_generate_v4(),
  tahun integer NOT NULL,
  kategori text DEFAULT 'BPD',
  budget_awal numeric(15,2) DEFAULT 0,
  budget_sisa numeric(15,2) DEFAULT 0,
  created_at timestamptz DEFAULT NOW(),
  updated_at timestamptz DEFAULT NOW(),
